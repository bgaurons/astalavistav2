 ...
 *		<ul>
 *			<li>Remodified the method for sending out points to plot.</li>
 *		</ul>
 *		<li>November 03, 2008, Benjamin Gauronskas</li>
 *		<ul>
 *			<li>Added logic for sending points.</li>
 *		</ul>
...
//				case Message.POINT:		pointMapping((PointMessage) msg);break;
		...
		private void manual(ManMessage msg){
			Movement.calculateMan(msg.direction);
		}
...
//					case AI.ROAMIN: Registers.ai = new RoamAI(); break;
		...
		/**
		 * Updates the server's map with a complete copy of the map.
		 *
		 *
		...
		/**
		 * Updates the server's map with a point.
		 *
		 *
		 * @param	msg		The message containing the updated map points.
		 * @author			Benjamin Gauronskas
		 */
		private void pointMapping(PointMessage msg){  // used to have PointMessage 
			Registers.map.updateMapPoint(msg.point, msg.color);
		}

		/**
		 * Updates the server's map with a segment.
		 *
		 *
		 * @param	msg		The message containing the updated segments.
		 * @author			Benjamin Gauronskas
		 */
		private void SegmentMapping(SegmentMessage msg){
			Registers.map.plotSegment(msg.start, msg.end, msg.target);
		}

		/**
		 * Updates the server's map with a heat plot.
		 *
		 *
		 * @param	msg		The message containing the heat plot.
		 * @author			Benjamin Gauronskas
		 */
