	...
	ControllerListener
{
    ...
    //---------------- main -------------------------------------------
    //	The main send a UDP packet to request the video transfer
    //-----------------------------------------------------------------
    public static void main(String argv[])
	{
		...
		try
		{
			...
			//Create the object clientSocket without contacting host server
			DatagramSocket clientSocket = new DatagramSocket();

			//get IP address of destination host
			InetAddress IPAddress =	InetAddress.getByName(IPA);

			//Stores data the client will send
			byte[] sendData = new byte[1024];

			//store user input to sendData
			sendData = temp.getBytes();
			//Construct the packet to put in the network through
			//its socket
			DatagramPacket sendPacket =
			new DatagramPacket(sendData, sendData.length, IPAddress, 9874);
			//Send Packet to server
			clientSocket.send(sendPacket);
		}
		catch(Exception e)
		{
			System.out.println("Error: Unable to send IP Address to robot.");
		}

		System.out.println("Sent UDP!");

		AVReceive2 avReceive = new AVReceive2(input);
